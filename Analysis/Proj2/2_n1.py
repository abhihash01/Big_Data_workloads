...
def main():
    ...
    if rank == 0:
        ...
        chunks_per_process = len(shuffled_data) // num_mappers
        remainder = len(shuffled_data) % num_mappers

        scattered_chunks = [shuffled_data[i*chunks_per_process + min(i, remainder):(i+1)*chunks_per_process + min(i+1, remainder)] for i in range(num_mappers)]

    else:
        scattered_chunks = None

    #local_data = comm.scatter(scattered_chunks if rank < num_mappers else None, root=0)
    local_data = comm.scatter(scattered_chunks, root=0)

    

    local_mapped_data = mapper(local_data) if rank < num_mappers else []

    #all_mapped_data = comm.gather(local_mapped_data if rank < num_mappers else None, root=0)
    all_mapped_data = comm.gather(local_mapped_data, root=0) if rank < num_mappers else None
    ...
    #local_keys = comm.scatter(scattered_keys if rank < num_reducers else None, root=0)
    local_keys = comm.scatter(scattered_keys, root=0)


    result = reducer(local_keys) if rank < num_reducers else []

    #all_results = comm.gather(result if rank < num_reducers else None, root=0)
    all_results = comm.gather(result, root=0) if rank < num_reducers else None
